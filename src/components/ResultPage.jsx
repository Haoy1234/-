...
const ResultPage = ({ data }) => {
  ...
  useEffect(() => {
    const preloadFonts = async () => {
      try {
        // 预加载Alexandria字体
        const font = new FontFace('Alexandria', 'url(https://fonts.gstatic.com/s/alexandria/v3/xn7gYHE41ni1AdIRggexSuXd.woff2)')
        await font.load()
        document.fonts.add(font)
        console.log('Alexandria字体预加载成功')
      } catch (error) {
        console.warn('字体预加载失败，将使用系统字体:', error)
      }
    }
    ...
    try {
      ...
      const canvas = await html2canvas(element, {
        ...
        onclone: (clonedDoc) => {
          ...
          if (clonedElement) {
            clonedElement.style.fontFamily = 'Alexandria, Microsoft YaHei, sans-serif'
          }
        }
      })
      ...
      link.download = `黄小仙八字分析_${timestamp}.png`
      link.href = imgData
      ...
      // 成功提示
      const successMsg = isSmallScreen ? 
        '✅ 高清图片已保存！\n如果没有自动下载，请长按图片保存到相册。' :
        '✅ 高清图片已保存到下载文件夹！'
      alert(successMsg)
  ...
  // 计算手机端缩放比例
  const getScaleRatio = () => {
    const padding = 20 // 左右各10px的边距
    const availableWidth = windowWidth - padding
    
    if (availableWidth < 595) {
      return Math.max(availableWidth / 595, 0.3) // 最小缩放比例0.3
    }
    return 1
  }

  const scaleRatio = getScaleRatio()
  const isSmallScreen = scaleRatio < 1
  ...
  return (
    <div className="min-h-screen bg-gray-100" style={{ 
      ...
      <button
        ...
        className="mb-4 bg-yellow-600 text-white px-6 py-3 rounded-lg shadow-lg hover:bg-yellow-700 transition-colors z-50"
        style={{
      ...
      <div 
        style={{
          ...
          marginBottom: isSmallScreen ? `${(842 * scaleRatio - 842) / 2}px` : '0'
        }}
      >
        ...
        <div 
          ...
          style={{
            ...
            borderRadius: '8px',
            fontFamily: 'Alexandria, "Microsoft YaHei", "PingFang SC", "Hiragino Sans GB", sans-serif',
